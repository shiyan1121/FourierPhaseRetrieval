...
def build_multi_step_scheduler(max_iter, values, milestones, warmup_iter=0):
    scheduler = MultiStepParamScheduler(values=values, milestones=milestones)

    if warmup_iter>0:
        scheduler = WarmupParamScheduler(
            scheduler=scheduler,
            warmup_length=warmup_iter/max_iter,
            warmup_factor=0.001,
...
def build_cosine_step_scheduler(max_iter, start, end, warmup_iter=0):
    end_value=end/start
    assert end_value>=0.0 and end_value<=1.0, end_value

    scheduler = CosineParamScheduler(start_value=1, end_value=end_value)
...
multi_step_scheduler = L(build_multi_step_scheduler)(
    max_iter = 80 * 100,
    values = [1.0, 0.1, 0.01],
    milestones = [80 * 50, 80 * 75, 80 * 100],
    warmup_iter = 400,
)

cosine_step_scheduler = L(build_cosine_step_scheduler)(
    max_iter = 80 * 100,
    start = 0.0001,
    end = 0.01 * 0.0001,
    warmup_iter = 400,
